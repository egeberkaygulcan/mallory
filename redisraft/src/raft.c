...
int serializeAEReq(raft_appendentries_req_t *msg, char **out_s) {
    ...
    if (msg->n_entries > 0) {
        ...
        snprintf(out, 100, "(%lf,%lf,%lf,%lf,%lu,%s)", 
            (double) msg->term, 
            (double) msg->prev_log_idx, 
            (double) msg->prev_log_term,
            (double) e->term,
            (long) msg->leader_commit,
            entry_data
        );
        *out_s=out;
    } else {
        snprintf(out, 100, "(%lf,%lf,%lf,%lu)", 
            (double) msg->term, 
            (double) msg->prev_log_idx, 
            (double) msg->prev_log_term,
            (long) msg->leader_commit
...
int serializeAEResp(raft_appendentries_resp_t *msg, char **out_s) {
    ...
    snprintf(out, 100, "(%lf,%d,%lf,%lf)", (double) msg->term, msg->success, (double) msg->current_idx, (double) msg->msg_id);
...
int serializeRVReq(raft_requestvote_req_t *msg, char **out_s) {
    ...
    char *out = malloc(100);
    snprintf(out, 100, "(%d,%lf,%lf,%lf,%lf)",
        msg->prevote,
        (double) msg->term,
        (double) msg->candidate_id,
        (double) msg->last_log_idx,
        (double) msg->last_log_term
    );
...
int serializeRVResp(raft_requestvote_resp_t *msg, char **out_s) {
    ...
    snprintf(out, 100, "(%lf,%d,%lf,%d)",
        (double) msg->term,
        msg->prevote,
        (double) msg->request_term,
        msg->vote_granted
    );
